...
（2） 产品详情页（参考淘宝）
（3） 购物车、下单页
（4） 个人中心：订单、购物车、评价、退换货、消息中心、优惠券。

（1）商品管理：商品分类查看及管理；商品列表，查看商品名称、分类、价格、库存等信息，可添加新商品；商品类型管理；商品规格管理；商品属性管理；品牌列表，查看已有品牌及分类，可添加品牌；商品评论管理，查看用户对商品的评论内容，可选择是否在页面显示。
（2）订单管理：订单列表、订单查询、添加订单、发货单列表、退货单列表、退货申请列表。
（3）物流设置：物流接口设置；
（4）资金记录：资金类型分类显示，
（5）优惠券：
     来源：后台管理员手动发放（单个或多选用户），优惠券和钱是1比1的。
     去向：用于所有消费减免（满减？）。
     配置：优惠券总开关，商家在商品管理中对优惠券的设置。
     管理：优惠券列表，使用记录。
（6）积分：
     来源：用户消费后返给用户的积分，消费一元得一积分。
     去向：用于兑换服务商城的配件或服务。
     配置：商家在服务商城的商品管理中的设置，设置多少积分兑换,为 0 则不能换。
     管理：积分总量、剩余，积分消费记录
...
商品发布流程：
    1、先选分类
    2、填写基本信息：名称、简介、售价、市场价、成本、库存
    3、商品规格：
    4、商品详情：品牌、属性、详情、是否虚拟商品、
    5、物流运费：
    6：其它信息：推荐、积分

设计思路：
    商品分类关联类型 ，类型关联规格和属性。规格用于商品组合属性SKU。属性用于商品筛选、商品发布页属性选择、商品详情页展示。

商城配置：积分开关、优惠券开关
商品品牌：配件的品牌不同于汽车的品牌
商品分类：分类的管理
    设置：设置推荐品牌、别名、广告位
    编辑：上级分类（编辑中暂时不显示）、分类名称、logo、发布虚拟（关联到子分类、是否）、类型选择（关联到子分类，单选）、

类型管理：类型名、所属分类、关联规格、关联品牌、排序、
          添加属性：属性名、属性值：多个值用英文“,”隔开（）、移除、编辑
    ㈠ 设置属性：用于属性分类，在商品分类中选取一个类型后，商品发布时只显示该类型的 规格和属性。在商品筛选页显示该类型下添加的属性。
    ㈡ 选定预先添加的规格用于SKU
    ㈢ 所属分类：指定商品的某一分类
    ㈣ 所属品牌：指定某一品牌
    ㈤
规格管理：规格名、所属分类、排序。
    ...
    影响商品的市场价、售价、库存、主图。
    用于商品组合属性SKU。
商品属性：属性名、属性值、所属类型、状态、排序。
    比如：颜色、材质、重量。
    用于商品筛选、商品发布页属性选择、商品详情页展示。 
图片空间：
    图片管理。
...
其它表设计：
    ...
    配件品牌表(cmf_shop_brand)：
        (PK)品牌ID、名称、logo、所属分类、推荐(is_rec)、状态、排序
    订单表(cmf_shop_order)：
