...
class WxPayApi
{
    ...
    public function __construct($p_set)
    {
        self::$p_set = $p_set;
        new WxPayData($p_set);
        new WxPayDataBase($p_set);
        WxPayResults::$p_set = $p_set;
        WxPayReport::$p_set = $p_set;
    }
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(self::$p_set['NOTIFY_URL']);//异步通知url
		}

		$inputObj->SetAppid(self::$p_set['APPID']);//公众账号ID
		$inputObj->SetMch_id(self::$p_set['MCHID']);//商户号
	...
	{
		//如果不需要上报数据
		if(self::$p_set['REPORT_LEVENL'] == 0){
			return;
		} 
		//如果仅失败上报
		if(self::$p_set['REPORT_LEVENL'] == 1 &&
	...
	{		
		...
		//如果有配置代理这里就设置代理
		if(self::$p_set['CURL_PROXY_HOST']!="0.0.0.0" && self::$p_set['CURL_PROXY_PORT']!=0){
			curl_setopt($ch,CURLOPT_PROXY, self::$p_set['CURL_PROXY_HOST']);
			curl_setopt($ch,CURLOPT_PROXYPORT, self::$p_set['CURL_PROXY_PORT']);
		}
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, self::$p_set['SSLCERT_PATH']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, self::$p_set['SSLKEY_PATH']);
		}
		//post提交方式
