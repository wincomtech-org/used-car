...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($this->p_set['NOTIFY_URL']);//异步通知url
		}
		
		$inputObj->SetAppid($this->p_set['APPID']);//公众账号ID
		$inputObj->SetMch_id($this->p_set['MCHID']);//商户号
	...
	{
		//如果不需要上报数据
		if($this->p_set['REPORT_LEVENL'] == 0){
			return;
		} 
		//如果仅失败上报
		if($this->p_set['REPORT_LEVENL'] == 1 &&
	...
	{		
		...
		//如果有配置代理这里就设置代理
		if($this->p_set['CURL_PROXY_HOST']!="0.0.0.0" && $this->p_set['CURL_PROXY_PORT']!=0){
			curl_setopt($ch,CURLOPT_PROXY, $this->p_set['CURL_PROXY_HOST']);
			curl_setopt($ch,CURLOPT_PROXYPORT, $this->p_set['CURL_PROXY_PORT']);
		}
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, $this->p_set['SSLCERT_PATH']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, $this->p_set['SSLKEY_PATH']);
		}
		//post提交方式
